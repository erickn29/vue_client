    ...
    renderVacanciesList = function (data) {
        ...
        for (let i = 0; i < data.results.length; i++) {
            ...
            <div class="vacancy-experience">
                опыт ${data.results[i].experience}
            </div>
            `
    
            let stack = ''
            if (data.results[i].stack) {
                stack += '<div class="stack">'
                for (let j = 0; j < data.results[i].stack.length; j++) {
                    stack += `
                    <span class="stack-item">${data.results[i].stack[j]}</span>
                    `
                }
            }
            ...
            <div class="company-name">
                ...
                ${data.results[i].company.name}
            </div>
            `
    
            let city = ''
            if (data.results[i].company.city){
                city = `
